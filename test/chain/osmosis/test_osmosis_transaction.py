...
class TestOsmosisTransaction(unittest.TestCase):
  def test_get_timestamp(self):
    swap_transaction = json.loads(Path('%s/../../testdata/chain/osmosis/swap.json' % os.path.dirname(__file__)).read_text())
  ...
  def test_get_transaction_mepty_fee_list(self):
    swap_transaction = json.loads(Path('%s/../../testdata/chain/osmosis/empty_fee_list.json' % os.path.dirname(__file__)).read_text())
    transaction = OsmosisTransaction(swap_transaction)
    fee = transaction.get_transaction_fee()
    self.assertEqual(fee, Decimal('0'))
